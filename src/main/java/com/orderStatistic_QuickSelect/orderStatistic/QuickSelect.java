...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelect(T[] array, int k) {                  
		...
		int resultado = busca(array, k, left, rigth);         
		                                                   
		return array[resultado];
	}

	private int busca(T[] array, int k, int left, int right) {
		...
		if(k - 1 < pivot) {
			return busca(array, k, left, pivot - 1);
		} else {
			return busca(array, k, pivot + 1, right);                  
		}
	}
	...
	public int quickSelectIndex(T[] array, int k) {                  
